...
from .http_clients.async_http import async_fetch_response, async_req
from .javascript import call_js_file, compile_js, run_js_cli_file
from .logger import script_path
from .models import make_stream_info, normalize_platform_payload
from .room import UnsupportedUrlError, get_sec_user_id, get_unique_id
...
async def get_douyin_web_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None):
    ...
    return normalize_platform_payload(room_data)
...
async def get_kuaishou_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    try:
        ...
        return make_stream_info(type=1, is_live=False)
    ...
    result = make_stream_info(type=2, is_live=False)
    ...
    if play_list["liveStream"].get("playUrls"):
        ...
        result["flv_url_list"] = play_url_list
        result["is_live"] = True

    return normalize_platform_payload(result)
...
async def get_kuaishou_stream_data2(
    ...
    try:
        ...
        result = make_stream_info(type=2, anchor_name=anchor_name, is_live=False)
...
async def get_huya_app_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    if live_status != "ON":
        return make_stream_info(anchor_name=anchor_name, is_live=False)
    else:
...
async def get_douyu_info_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    json_data = json.loads(json_str)
    result = {"anchor_name": json_data["room"]["nickname"], "is_live": False}
...
async def get_yy_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    return normalize_platform_payload(json_data)
...
async def get_bilibili_room_info(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    try:
        ...
        return normalize_platform_payload(
            {
                "anchor_name": anchor_name,
        ...
        print(e)
        return normalize_platform_payload({"anchor_name": "", "live_status": False, "room_url": url})
...
async def get_xhs_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    result = make_stream_info(anchor_name="", is_live=False)
    html_str = await async_req(url, proxy_addr=proxy_addr, headers=headers)
...
async def get_bigo_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    result = make_stream_info(anchor_name=anchor_name, is_live=False)
...
async def get_blued_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    if live_status:
        ...
        result = make_stream_info(result, is_live=True, m3u8_url=m3u8_url, record_url=m3u8_url)
    return normalize_platform_payload(result)
...
async def _fetch_web_stream_data_global(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    result = make_stream_info(anchor_name=anchor_name or "", is_live=False, live_url=url)
...
async def get_sooplive_stream_data(
    ...
    result = make_stream_info(anchor_name=anchor_name or "", is_live=False)
...
async def get_netease_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    result = make_stream_info(is_live=False)
...
async def get_pandatv_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    if live_status:
        ...
        result = make_stream_info(result, is_live=True, m3u8_url=play_url, play_url_list=play_url_list)
    return normalize_platform_payload(result)
...
async def get_winktv_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    result = make_stream_info(anchor_name=anchor_name, is_live=live_status)
    if live_status:
        ...
        result = make_stream_info(result, m3u8_url=m3u8_url, play_url_list=play_url_list, is_live=True)
    return normalize_platform_payload(result)
...
async def get_flextv_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> str:
        ...
        if live_status:
            ...
            if play_url:
                if ".m3u8" in play_url:
                    ...
                    result = make_stream_info(result, is_live=True, flv_url=play_url, record_url=play_url)
        else:
...
async def get_popkontv_stream_url(
    ...
    if room_info:
        ...
        if "HTTP Error 400" in json_str or 'statusCd":"E5000' in json_str:
            print(
                "Failed to retrieve popkontv live stream [token does not exist or has expired]: Please log in to watch."
            )
            print(
...
async def get_liveme_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    sign_data = call_js_file("liveme.js", "sign", room_id, f"{JS_SCRIPT_PATH}/crypto-js.min.js")
...
async def get_changliao_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    async def get_live_domain(page_url):
        ...
        config_json_str = (
            re.findall("var config = (.*?)config.webskins", html_str, re.DOTALL)[0].rsplit(";", maxsplit=1)[0].strip()
        )
...
async def get_haixiu_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    ajax_data = call_js_file("haixiu.js", "sign", params, f"{JS_SCRIPT_PATH}/crypto-js.min.js")
...
async def get_taobao_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    for i in range(2):
        ...
        sign = call_js_file("taobao-sign.js", "sign", pre_sign_str)
        ...
        jsonp_str, new_cookie = await async_req(
...
async def get_jd_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    params = {"body": '{"liveId": "' + live_id + '"}', "functionId": "getImmediatePlayToM", "appid": "h5-live"}
...
async def get_migu_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    if live_status != "1":
        ...
        ddCalcu = run_js_cli_file("migu.js", source_url)
...
async def get_laixiu_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    sign_data = call_js_file("laixiu.js", "sign", f"{JS_SCRIPT_PATH}/crypto-js.min.js")
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) "
