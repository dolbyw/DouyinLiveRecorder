...
from .models import make_stream_info
...
async def get_tiktok_stream_url(json_data: dict, video_quality: str, proxy_addr: str) -> dict:
    if not json_data:
        return make_stream_info(anchor_name=None, is_live=False)
...
async def get_kuaishou_stream_url(json_data: dict, video_quality: str) -> dict:
    if json_data["type"] == 1 and not json_data["is_live"]:
        return make_stream_info(json_data)
    live_status = json_data["is_live"]

    result = make_stream_info(type=2, anchor_name=json_data["anchor_name"], is_live=live_status)
...
async def get_stream_url(
    ...
    url_type: str = "m3u8",
    spec: bool = False,
    hls_extra_key: str | int = None,
    flv_extra_key: str | int = None,
) -> dict:
    if not json_data["is_live"]:
        return make_stream_info(json_data)
    ...
    data = make_stream_info(anchor_name=json_data["anchor_name"], is_live=True)
