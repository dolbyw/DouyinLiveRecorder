...
- `录制结束` 会在录制完成后兜底触发上传；如果开启“按时间分段录制”并启用 TS 转 MP4，单个分段确认写入完成并成功转为 MP4 后也会立即触发上传。上传会排除 `.converting.mp4` 和仍作为转码源的 `.ts` 分段，避免移动正在写入或正在转码的文件。
- `最小文件冷却时间` 仅用于定时/间隔扫描，避免移动仍在写入的文件；录制结束触发会直接处理已确认完成的文件。
